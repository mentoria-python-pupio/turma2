...
import argparse
import threading
from telegram_bot import envia_msg_telegram
...
def get_json_data_from_chaos(only_bount: bool = True):
    ...
    if response.status_code != 200:
        envia_msg_telegram("PROBLEMA AO OBTER DADOS DO CHAOS!!")
...
def executa_chaos(plataformas, threads):
    dados = get_json_data_from_chaos()
    envia_msg_telegram("Terminei de baixar todas as URLs dos ZIPs")
    ...
    for plataforma in plataformas:
        envia_msg_telegram(f"INICIANDO DOWNLOAD DOS DADOS DA {plataforma}")
        ...
        envia_msg_telegram(f"Terminei de baixar os itens da {plataforma}")
...
if __name__ == "__main__":
    ...
    executa_chaos(plataformas, threads)
